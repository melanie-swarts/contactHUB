...
export default {
  ...
  methods: {
      ...
      if (this.$refs.calendarEventTypeForm.$refs.form.validate()) {
        if (!this.calendarEventTypeExists(this.newCalendarEventTypeInfo)) {
          this.$store.dispatch('calendarEventTypes/createCalendarEventType', this.newCalendarEventTypeInfo)
            .then(() => {
              this.$store.dispatch('snackbar/setSnack', this.successMessage(true, this.newCalendarEventTypeInfo.Name + ' was successfully created.', 'success'))
              this.$router.push({
                name: 'Lookups'
              })
          ...
          this.$store.dispatch('snackbar/setSnack', this.successMessage(true, 'Calendar Event Type ' + this.newCalendarEventTypeInfo.Name + ' already exists.', 'error'))
        }
      }
    },
    ...
    calendarEventTypeExists(newCalendarEventTypeInfo) {
      if (this.$store.state.calendarEventTypes.calendarEventTypes.filter(x => x.Name.toUpperCase() === newCalendarEventTypeInfo.Name.toUpperCase()).length > 1) {
        return true
      }
      return false
    },
